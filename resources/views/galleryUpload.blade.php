...
<body>
    <div class="container-fluid mt-2">
        ...
        <div class="container mt-2">
            ...
            <form method="POST" action="{{ route('imageGalleryPost') }}" enctype="multipart/form-data">
                @csrf
                <div class="form-group mt-2">
                    <label for="eventYear">Picture Year</label>
                    <select class="form-control form-control-md" name="eventYear" id="eventYear" aria-describedby="eventYear" required>
                        @foreach (range(2000,date("Y")) as $key => $year)
                        <option value="{{$year}}" <?php echo $key === date("Y") && "selected"; ?>>{{$year}}</option>
                        @endforeach
                    </select>
                </div>
